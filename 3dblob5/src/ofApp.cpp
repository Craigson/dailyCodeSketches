...
//    }
    ...
    for (int i = 0; i < particles.size(); i++){
        ...
        particles[i]->addAttractionForce(0,0,0,1000,1.3);
    }
//
float f = ofGetElapsedTimef();
    for (int i = 0; i < particles.size(); i++){
        ...
        p.setPoint(particles[i]->pos);
         ...
         float x1 = (sin(particles[i]->lat*0.01 + f*2 + particles[i]->lon*0.3)) * 400 + 400;
        
        int counter = 0;
        ann_points::result_knn neighbors = ann_allPoints->findNeighbors(particles[i]->pos, ofMap(sin(f), -1,1, 20,100));
             ...
             neighbors.indices_it++)
        {
            ...
            float y1 = (cos(particles[j]->lat*0.01 - f*1.5 + PI/9.0)) * 200  + 400;

            
            particles[i]->addRepulsionForce( *particles[j], MAX(x1 + y1, 1), 0.3);
